...
public class OrderDao {
	...
	public ArrayList<OrderViewVO> selectOrdersByOseq(int oseq) {
		...
		public ArrayList<OrderViewVO> selectOrdersById(String id) {
			ArrayList<OrderViewVO> list=new ArrayList<>();
			con=Dbman.getConnection();
			String sql="select *from order_view where id=? order by oseq desc";
			try {
				pstmt=con.prepareStatement(sql);
	...
	public ArrayList<OrderViewVO> selectOrdersIngById(String id) {
		ArrayList<OrderViewVO> list=new ArrayList<>();
		con=Dbman.getConnection();
		String sql="select *from order_view where id=? and result in (0,1) order by oseq desc";
		try {
			pstmt=con.prepareStatement(sql);
			pstmt.setString(1, id);
			rs=pstmt.executeQuery();
